...
// The two outer-boundary diagnostics moved to CapyrX_TestOuterBC, and
// activating that thorn is now the switch.  `run_outer_bc_tests` stays in this
// thorn because 182 existing par files set it; without this check they would
// parse, run, and quietly produce no diagnostic output at all, which is the
// worst of the three possible outcomes.  Refuse instead, by name, with the
// line to add.
extern "C" void CapyrX_TestMultiPatch_Check_Parameters(CCTK_ARGUMENTS) {
  DECLARE_CCTK_PARAMETERS;

  if (run_outer_bc_tests && !CCTK_IsThornActive("CapyrX_TestOuterBC"))
    CCTK_VERROR(
        "CapyrX_TestMultiPatch::run_outer_bc_tests is set, but thorn "
        "CapyrX_TestOuterBC is not active. The NaN-injection and smooth-field "
        "outer-boundary diagnostics, and the four grid functions they write, "
        "now live in that thorn: this thorn declares only grid functions it "
        "always writes, because CarpetX allocates a MultiFab for every "
        "DECLARED grid function and requires every one of them to be valid "
        "before it creates a refined level, so a field written only by a "
        "switched-off diagnostic aborts any run with "
        "CarpetX::max_num_levels > 1. Add CapyrX_TestOuterBC to ActiveThorns, "
        "and rename CapyrX_TestMultiPatch::{nan_injection_mode,smooth_field,"
        "smooth_sentinel} to CapyrX_TestOuterBC::... if this parameter file "
        "sets them.");
}
