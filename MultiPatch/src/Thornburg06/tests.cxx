...
std::string patch_owner_test(const PatchTransformations &pt,
                             const Thornburg06::svec &x,
                             Thornburg06::patch_piece expected) {

  using namespace MultiPatch::Thornburg06;
...
std::string global_identity_test(const PatchTransformations &pt,
                                 const Thornburg06::svec &global_vars) {
...
std::string local_identity_test(const PatchTransformations &pt, int patch,
                                const Thornburg06::svec &local_point) {

  using MultiPatchTests::colored;
  using MultiPatchTests::isapprox;
  using MultiPatchTests::string_color;
  using Thornburg06::patch_piece;
  using Thornburg06::piece_name;
...
std::string jacobian_test(const PatchTransformations &pt, int patch,
                          const Thornburg06::svec &global_point) {
 ...
 */
extern "C" void MultiPatch_run_thornburg06_tests(CCTK_ARGUMENTS) {
  ...
  using MultiPatch::Thornburg06::get_owner_patch;
  using MultiPatch::Thornburg06::patch_piece;
  using MultiPatch::Thornburg06::piece_name;
  using MultiPatch::Thornburg06::svec;

  using MultiPatch::dim;
  using MultiPatch::SetupThornburg06;
  using MultiPatchTests::random_seed;

  using namespace MultiPatch::Thornburg06Tests;

  const auto ps = SetupThornburg06();
  ...
  CCTK_INFO("Running Thornburg06 patch tests:");

  /*
   * Fixed point tests
   */
  const std::array<std::pair<svec, patch_piece>, 24> owner_test_data = {
  ...
  uniform_real_distribution<CCTK_REAL> r_distrib(r0, r1);
  ...
  uniform_int_distribution<int> patch_distrib(0, 5);
